...
class StreamSessionRuntime {
  ...
  async transition(session, toState, eventType, metadata = {}, failure = {}) {
    const updated = await this.sessionRepository.transition({
  ...
  assertGeneration(session, command) {
    if (session.generation !== command.generation) {
      ...
      error.code = 'STALE_WORKER_GENERATION';
      throw error;
    }
  }
  ...
  async start(command) {
    ...
    if (!session) {
      const error = new Error('Stream session not found');
    ...
    if (existing) {
      if (existing.leaseTokenHash !== leaseTokenHash || existing.generation !== command.generation) {
        const error = new Error('Another lease already owns this local session');
        error.code = 'LOCAL_WORKER_LEASE_CONFLICT';
        throw error;
      }
    ...
    if (TERMINAL_STATES.has(session.state)) {
      const error = new Error(`Cannot start terminal stream session ${session.state}`);
      error.code = 'STREAM_SESSION_TERMINAL';
      throw error;
    }

    if (session.state === STREAM_STATE.QUEUED) {
      session = await this.transition(session, STREAM_STATE.ALLOCATING, 'WORKER_V1_SELF_ALLOCATED', {
        workerIdentity: this.workerIdentity,
        compatibilityMode: true
      });
    }
    if (session.state !== STREAM_STATE.ALLOCATING) {
      const error = new Error(`START_SESSION requires ALLOCATING state, received ${session.state}`);
      error.code = 'STREAM_SESSION_NOT_ALLOCATING';
      throw error;
    }

    const leaseExpiresAt = new Date(Date.now() + this.leaseDurationMs).toISOString();
    session = await this.sessionRepository.claimLease({
      workspaceId: command.workspaceId,
      sessionId: command.sessionId,
      generation: command.generation,
      leaseTokenHash,
      leaseExpiresAt,
      workerRuntime: { workerIdentity: this.workerIdentity, pid: process.pid, phase: 'claimed' }
    });
    ...
    session = await this.transition(session, STREAM_STATE.STARTING, 'WORKER_STARTING', {
      workerIdentity: this.workerIdentity,
      executable: mediaCommand.executable,
      args: mediaCommand.redactedArgs
    });
    ...
    try {
      const processInfo = this.processSupervisor.start({
        sessionId: command.sessionId,
        executable: mediaCommand.executable,
        args: mediaCommand.args,
        onProgress: () => this.markLive(command.sessionId),
        onLog: () => {},
        onExit: (result) => this.handleProcessExit(command.sessionId, result)
      });

      session = await this.transition(session, STREAM_STATE.CONNECTING, 'WORKER_PROCESS_STARTED', {
        workerIdentity: this.workerIdentity,
        pid: processInfo && processInfo.pid ? processInfo.pid : null
      });
      ...
      if (current && !TERMINAL_STATES.has(current.state)) {
        try {
          await this.transition(current, STREAM_STATE.FAILED, 'WORKER_START_FAILED', {}, {
  ...
  startHeartbeat(entry) {
    const tick = async () => {
      try {
        ...
        await this.sessionRepository.heartbeat({
          workspaceId: entry.workspaceId,
          sessionId: entry.sessionId,
          generation: entry.generation,
          leaseTokenHash: entry.leaseTokenHash,
          leaseExpiresAt: new Date(Date.now() + this.leaseDurationMs).toISOString(),
          workerRuntime: {
            workerIdentity: this.workerIdentity,
            pid: processInfo.pid || null,
            lastActivityAt: processInfo.lastActivityAt || null
          }
        });
      } catch (error) {
        if (error && error.code === 'STREAM_SESSION_LEASE_LOST') {
          await this.processSupervisor.stop(entry.sessionId).catch(() => {});
        }
      }
    };
    entry.heartbeatTimer = setInterval(() => { tick().catch(() => {}); }, this.heartbeatIntervalMs);
    if (typeof entry.heartbeatTimer.unref === 'function') entry.heartbeatTimer.unref();
  }

  async markLive(sessionId) {
    const entry = this.active.get(sessionId);
    if (!entry || entry.liveMarked) return;
    entry.liveMarked = true;
    const session = await this.sessionRepository.findById(entry.workspaceId, sessionId);
    if (!session || session.generation !== entry.generation || session.state !== STREAM_STATE.CONNECTING) return;
    await this.transition(session, STREAM_STATE.LIVE, 'WORKER_MEDIA_FLOWING', { workerIdentity: this.workerIdentity });
  }

  async stop(command) {
    const session = await this.sessionRepository.findById(command.workspaceId, command.sessionId);
    if (!session) return { stopped: false, reason: 'not_found' };
    this.assertGeneration(session, command);
    const expectedHash = hashLeaseToken(command.leaseToken);
    if (session.lease_token_hash && session.lease_token_hash !== expectedHash) {
      const error = new Error('STOP_SESSION lease token does not own this session');
      error.code = 'STREAM_SESSION_LEASE_CONFLICT';
      throw error;
    }
  ...
  async stopSession(session, reason) {
    let current = session;
    if (TERMINAL_STATES.has(current.state)) return { stopped: true, state: current.state };
    if (current.state !== STREAM_STATE.STOPPING) {
      current = await this.transition(current, STREAM_STATE.STOPPING, 'WORKER_STOPPING', { reason, workerIdentity: this.workerIdentity });
    }
    ...
    this.clearActive(current.id);
    return { stopped: true, state: current.state };
  }
  ...
  async handleProcessExit(sessionId, result) {
    ...
    await this.transition(session, STREAM_STATE.FAILED, 'WORKER_PROCESS_EXITED', {
      code: result.code,
      signal: result.signal
    }, {
      code: result.error ? 'FFMPEG_PROCESS_ERROR' : 'FFMPEG_PROCESS_EXIT',
      message: result.error ? result.error.message : `FFmpeg exited with code ${result.code}`
    });
  }

  clearActive(sessionId) {
    const entry = this.active.get(sessionId);
    if (entry && entry.heartbeatTimer) clearInterval(entry.heartbeatTimer);
  ...
  async shutdown() {
    ...
    for (const entry of entries) {
      const session = await this.sessionRepository.findById(entry.workspaceId, entry.sessionId).catch(() => null);
      if (session && !TERMINAL_STATES.has(session.state)) {
        await this.stopSession(session, 'worker_shutdown').catch(() => {});
      }
    }
